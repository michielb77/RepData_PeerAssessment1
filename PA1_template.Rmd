...
get_median_steps <- function(interval) {
  median_steps_per_interval$steps[which(median_steps_per_interval$interval == interval)]
} 

imp <- activity %>%
  mutate(steps = ifelse(is.na(steps), get_median_steps(interval), steps))
```

**TODO: this doesn't work yet. Some missing values are filled out, but a lot of them are not. Fix this, then create histogram and such**
...
activity <- activity %>% mutate(day_type = day_type(date))

activity %>%
  group_by(interval, day_type) %>%
  summarise(steps = mean(steps, na.rm = TRUE)) %>% # TODO: fix data and remove na.rm = TRUE
